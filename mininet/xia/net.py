...
class Mininet_xia( Mininet ):
    ...
    def __init__( self, topo=None, switch=OVSKernelSwitch, host=Host,
                  ...
                  listenPort=None, waitConnected=False,xia=Node_xia ):
	...
	for node in self.xias:
		...
		if 'hidneigh' in node.params:
			ppalid = node.params['hidneigh'].get('ppalid')
			intf = node.params['hidneigh'].get('intf')
			intf = intf.lstrip()
    ...
    def __iter__( self ):
        ...
        for node in chain( self.hosts, self.switches, self.controllers, self.xias ):
            yield node.name

    def __len__( self ):
        "returns number of nodes in net"
        return ( len( self.hosts ) + len( self.switches ) +
                 len( self.controllers ) + len( self.xias ) )
    ...
    def build( self ):
        ...
        info( '*** Configuring xia nodes\n' )
        self.configureXianodes()
	...
	self.terms += makeTerms( self.xias, 'xia' )
    ...
    def stop( self ):
        ...
        for switch in self.switches:
            ...
            switch.terminate()
        info( '\n' )
        info( '*** Stopping %i hosts/xia nodes\n' % (len( self.hosts ) + len( self.xias )) )
        for host in self.hosts + self.xias:
            info( host.name + ' ' )
            host.terminate()
        info( '\n*** Done\n' )
